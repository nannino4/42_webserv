...
{
	std::cout << "THE REQUEST FROM FD " << client.connected_fd << " IS:\n" << client.message << std::endl;	//DEBUG
	client.message = std::string("ciao, sono una response");	//DEBUG
	...
	struct kevent event;
	EV_SET(&event, client.connected_fd, EVFILT_WRITE, EV_ADD, 0, 0, (void *)&client);		// ident = connected_fd
